...
import SOWImportPanel from "./SOWImportPanel";
...
export default function WorkBuilder({ projectId, workTypes, crew }: Props) {
  ...
  const [showSowImport, setShowSowImport] = useState(false);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Import SOW button + panel */}
      {showSowImport ? (
        <SOWImportPanel
          projectId={projectId}
          onComplete={() => {
            setShowSowImport(false);
            router.refresh();
          }}
          onClose={() => setShowSowImport(false)}
        />
      ) : (
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => setShowSowImport(true)}
            className="flex items-center gap-1.5 rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-sm font-medium text-violet-700 hover:bg-violet-100 transition-colors"
          >
